...
async fn main() -> Result<()> {
    ...
    //innder data db type is PgPool, and PgPool is wrapped by Arc, so we can clone it with cheap cost
    //otherwise, we must wrap it with Arc<Mutex<PgPool>> to share it between threads
