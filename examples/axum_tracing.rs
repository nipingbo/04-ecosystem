...
async fn index_handler(req: Request) -> &'static str {
    ...
    info!(http.status_code = 200, "index handler finished");
    ret
}
...
async fn long_task() -> &'static str {
    ...
    // below 4 lines code runs sequentially
    //sleep(Duration::from_millis(112)).await;
    //task1().await;
    //task2().await;
    //task3().await;

    // now use join! macro to run multiple tasks concurrently
    // it optimizes the code to run concurrently
